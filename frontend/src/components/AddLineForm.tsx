...
export function AddLineForm({ departmentId: propDepartmentId, onSuccess, onCancel }: AddLineFormProps) {
  const [name, setName] = useState('');
  const [departmentId, setDepartmentId] = useState(propDepartmentId || '');
  ...
  const fetchDepartments = async () => {
    try {
      ...
      setDepartments(
        (data || []).map((d: any) => ({ id: d._id || d.id, name: d.name }))
      );
    } catch (error) {
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input
            ...
            placeholder="e.g., Assembly Line 1, Production Line A"
          />
        </div>
