...
export function Locations({ refresh }: LocationsProps) {
  ...
  const fetchLocations = async () => {
    try {
      ...
      const departmentsWithHierarchy: Department[] = (data || []).map((dept: any) => ({
        id: dept._id || dept.id,
        name: dept.name,
        description: dept.description,
        status: dept.status,
        lines: (dept.lines || []).map((line: any) => ({
          id: line._id || line.id,
          name: line.name,
          description: line.description,
          status: line.status,
          department_id: dept._id || dept.id,
          stations: (line.stations || []).map((s: any) => ({
            id: s._id || s.id,
            name: s.name,
            description: s.description,
            status: s.status,
            line_id: line._id || line.id,
          })),
        })),
      }));
  ...
  const handleDelete = async (type: 'department' | 'line' | 'station', id: string) => {
    ...
    try {
      ...
      if (type === 'line') await api.deleteLine(id);
      if (type === 'station') await api.deleteStation(id);
      fetchLocations();
    } catch (error) {
